...
*/
	...
	$gdfont['style_file'] 			= 'font_style.php';			// Set relative path from font.php to your font style file (if you plan on using it)
...
*/
	...
	$gdfont['text']					= '';
	$gdfont['font']					= 'verdana';
	$gdfont['size']					= 12 * $gdfont["multiplyer"];
	$gdfont['color']				= '#FFF';
...
################################
	...
	if(substr($gdfont['font'],-4) != '.ttf')
		$gdfont['font'] .= '.ttf';
	foreach(array("size", "max_width", "leading", "padding", "vadj", "hadj") as $elem){
		if(is_string($gdfont[$elem])){
			$gdfont[$elem] = (string)((int) $gdfont[$elem] * $gdfont["multiplyer"]);
		} else {
			$gdfont[$elem] = $gdfont[$elem] * $gdfont["multiplyer"];
		}
	}
	return $gdfont;
}
...
function gdfont($debug_mode = false) {
	...
	if($debug_mode) {
		...
		$gdfont['color'] = 'FFF';
		$gdfont['background'] = '000';
		$gdfont['padding'] = 20*$gdfont["multiplyer"];
		$gdfont['cache'] = false;
		$gdfont['max_width'] = '2400';
		$gdfont['leading'] = 10*$gdfont["multiplyer"];
